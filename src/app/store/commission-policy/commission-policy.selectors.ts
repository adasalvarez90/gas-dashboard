...
import { State, adapter } from './commission-policy.state';
import * as fromAuth from 'src/app/store/auth';

// Store state functions
import { match } from '../store.state';

export const selectors = createFeatureSelector<State>('commissionPolicies');
...
export const selectedCommissionPolicy = createSelector(selectors, (state) => state.selected);
